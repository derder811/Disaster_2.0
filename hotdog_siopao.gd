...
func _ready():
	...
	if interaction_area:
		...
		print("ERROR: InteractionArea not found in hotdog and siopao")

func _on_interact():
	print("DEBUG: _on_interact called in hotdog_siopao.gd")
	print("Examining the hotdog and siopao...")
	...
	if player and player.has_method("trigger_item_self_talk"):
		...
		player.trigger_item_self_talk("hotdog_siopao")
	else:
	...
	if store_quest and store_quest.has_method("on_hotdog_siopao_interaction"):
		store_quest.on_hotdog_siopao_interaction()